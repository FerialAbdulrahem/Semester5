...
namespace M3_Application
{
    ...
    public partial class HR_Approve_Annual_Acc : System.Web.UI.Page
    {
        ...
        protected void btnProcess_Click(object sender, EventArgs e)
        {
            ...
            if (currentStatus != null && !currentStatus.Equals("Pending", StringComparison.OrdinalIgnoreCase))
            {
                lblMessage.Text = "Error: Leave request has already been processed.";
                return;
            }

            ProcessApproval("HR_approval_an_acc");
            LoadUpdatedRecord();

            if (gvResult.Rows.Count == 0)
            {
                lblMessage.Text = "This Request is not an Annual / Accidental Leave.";
            }
            else
            {
        ...
        void LoadUpdatedRecord()
        {
            ...
            FROM Leave L, annual_leave AL
            WHERE L.request_ID = AL.request_ID and AL.request_ID= @id
            UNION SELECT l.request_id , l.date_of_request,l.start_date,l.end_date,l.num_days,l.final_approval_status
            FROM Leave L, accidental_leave AC
            WHERE L.request_ID = AC.request_ID and AC.request_ID= @id", conn))
            {
